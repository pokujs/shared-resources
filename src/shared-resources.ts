...
const decodeEncoded = (enc: Record<string, unknown>): unknown => {
  ...
  if (!codec)
    throw new Error(
      `No codec registered for tag "${String(enc.t)}". ` +
        'Register it via resource.configure({ codecs }) in the resource file ' +
        'or pass it to sharedResources({ codecs }) for the parent process.'
    );
...
export const resource = {
  ...
  configure: (config: { codecs?: ArgCodec<any>[] }) => {
    if (config.codecs?.length) configureCodecs(config.codecs);
  },
} as const;
